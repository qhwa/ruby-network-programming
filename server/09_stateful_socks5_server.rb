...
class Server
  ...
  def start(bind: '0.0.0.0', port: nil)
    EM.run { EM.start_server bind, port, StatefulHandler }
  end
  ...
  module StatefulHandler
    include Socks5Handler

    def connect
      ip, _ = client_addr

      # to allow:    redis-cli set allowed:127.0.0.1 1
      # to disallow: redis-cli del allowed:127.0.0.1
      EM.defer -> { Redis.new.get "allowed:#{ip}" },
               -> (allowed) { allowed ? super : close_connection }
    end
